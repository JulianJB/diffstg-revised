...
from .ugnet_128_bodysys25_0825 import UGnet
...
class DiffSTG(nn.Module):
    ...
    def __init__(self, config: easydict):
        ...
        self.sample_strategy = self.config.sample_strategy # sample strategy
        self.device = config.device
        self.beta_start = config.get('beta_start', 0.0001)
        #self.beta_end = config.get('beta_end', 0.02)
        self.beta_end = config.get('beta_end', 0.1)
    ...
    def p_sample(self, xt: torch.Tensor, t:torch.Tensor, x_masked: torch.Tensor):
        """
        Sample from p(x_{t-1}|x_t, c)
        """
        #eps_theta = self.eps_model(xt, t, c) # c is the condition
        eps_theta = self.eps_model(xt, t, x_masked)  # c is the condition
    ...
    def p_sample_loop(self, x_masked_cond: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            for t in range(self.N, 0, -1):  #in paper, t should start from T, and end at 1
                ...
                if t > 0: x = self.p_sample(x, x.new_full((B,), t, dtype=torch.long), x_masked_cond)
        return  x

    def p_sample_loop_ddim(self, x_masked_cond: torch.Tensor):
        #x_masked, _, _ = c
        x_masked = x_masked_cond
        B, F, V, T = x_masked.shape
        ...
        xs, x0_preds = generalized_steps(x, seq, self.eps_model, self.beta, x_masked_cond, eta=1)
        return xs, x0_preds
    ...
    def evaluate(self, input: torch.Tensor, n_samples=2):
        #x_masked, _, _ = input
        x_masked = input
        B, F, V, T = x_masked.shape
        if self.sample_strategy == 'ddim_multi':
            ...
            xs, x0_preds = self.p_sample_loop_ddim(x_masked)
            x = xs[-1]
    ...
    def loss(self, x0: torch.Tensor, x_masked: torch.Tensor):
        """
        Loss calculation
        x0: (B, ...)
        c: The condition, x_masked is a masked torch tensor
        """
        #
        ...
        eps_theta = self.eps_model(xt, t, x_masked)
...
def generalized_steps(x, seq, model, b, x_masked, **kwargs):
    with torch.no_grad():
        n = x.size(0)
...
def save2file(params):
    ...
    file_name = ws + f'/output/HAR/metrics/DiffSTG.csv'
    head = [
        # data setting
        'data.name',
        # model parameters
        ...
        # metric result
        'mae', 'rmse', 'mape', 'crps', 'mis', 'time', 'model_path', 'log_path', 'imputation_path',
    ]
